...
| `rdd_disabled` | Inspect the exact source-scoped `gentle-ai review mode enable` command rendered by bound facade STATUS. If its scope is global, STOP and obtain explicit human confirmation for that exact global mutation before running it. Only after any required confirmation, run the exact command, then `S`. |
