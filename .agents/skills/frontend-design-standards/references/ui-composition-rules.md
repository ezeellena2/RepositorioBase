...
`sx` used only for layout. Copy the shape, not the words: the copy on any given screen already exists and stays.
...
>
  <Box>
    ...
    <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5, maxWidth: 640 }}>
      {/* the screen's existing supporting sentence */}
    </Typography>
  </Box>
  <Button variant="contained" component={RouterLink} to="/members/invite">Invite a member</Button>
</Stack>
```
...
<TableContainer component={Paper} variant="outlined">
  <Table size="small">
    <TableHead>
      <TableRow>
        <TableCell component="th" scope="col">Member</TableCell>
...
const StatusChip = ({ status }) => (
  <Chip size="small" label={status} color={statusColor[status] ?? 'default'} variant="outlined" />
);
```

Keep the label exactly the domain word the screen already showed.
...
<Paper variant="outlined" sx={{ p: 4, textAlign: 'center' }}>
  <Typography variant="body2" color="text.secondary">
    {/* the screen's existing "nothing here yet" sentence */}
  </Typography>
...
The action is offered only when the viewer can actually perform it.
...
If an existing test asserts a `role="status"` element containing "Loading…", keep that text as the accessible
label and put the skeleton behind it. Never remove a role a test names.
...
<Paper variant="outlined" component="form" onSubmit={…} sx={{ p: { xs: 2, sm: 3 }, maxWidth: 560 }}>
  <Stack spacing={2}>
    <Typography component="h2" variant="subtitle1">Company details</Typography>
    <TextField
      id="register-cuit"
      label="CUIT"
      required
      ...
      helperText="Include the check digit."
      value={form.cuit}
...
7. Do the screen's tests still pass without being edited?
