...
- **IA-REQ-003:** organization registration deterministically follows the caller state below, and no branch reachable without proof of control of the submitted address creates an exclusive durable claim on a CUIT, identity, tenant, organization profile, membership or role.
  - Before caller-state branching or stored-state lookup, a submitted CUIT is normalized from ASCII digits, hyphens and whitespace and must contain exactly eleven digits with a valid AFIP modulo-11 verifier. The first ten digits use weights `5,4,3,2,7,6,5,4,3,2`; a result of `11` maps to verifier `0`, while result `10` is invalid. No CUIT prefix allowlist is applied. This is an input rule only: normalized eleven-digit rows written before the rule remain readable through the explicit stored-representation compatibility path and are not retroactively refused.
  - An anonymous request records one bounded, expiring `PendingRegistrationIntent`, enqueues one outbox message to the submitted address and returns the neutral `202`. It creates no identity, tenant, organization profile, membership, role or CUIT claim, and no state another caller can observe or be refused by. It behaves identically whatever the system knows of that address and that CUIT; only the delivered message differs — a confirmation link carrying the intent's single-use token when the address has no identity, and a tokenless sign-in notice when it already has one, which only that address can read. The public acknowledgement remains identical and links to `/login?returnUrl=%2Forganizations%2Fregister`; when no email arrives, it recommends only checking spam and waiting, because this flow exposes no resend action.
  - An authenticated request creates another organization for the current identity and derives its address from the validated session. A blank submitted email, including one containing only whitespace, is treated as absent; any nonblank submitted email must normalize to the session address or the request is rejected, and a session the server cannot validate is rejected as invalid rather than treated as anonymous. The unused submitted password is ignored. The new tenant and responsible membership belong to the authenticated identity only. A request whose normalized CUIT already belongs to an organization creates nothing and returns `409` `registration_conflict`: that caller may register only for the address their own session proves, so nothing they can vary asks about anybody else — and it can no longer be composed with an anonymous probe, because the anonymous phase leaves no claim for this answer to depend on.
...
- **IA-REQ-005:** email must be confirmed before inviting members, administering roles, accepting an invitation, or performing a sensitive operation. Every confirmation envelope is classified by its exact message type before expiry or another terminal state is interpreted. Four purposes — anonymous organization registration (`identity.registration.confirmation.requested`), authenticated/pre-upgrade organization registration (`identity.confirmation.requested`), Personal registration (`identity.personal.confirmation.requested`) and invited-member registration (`identity.invitation.confirmation.requested`) — spend their delivered link against `POST /api/identity/confirm-email`; the fifth, Platform invitation registration (`platform.invitation.confirmation.requested`), spends it against `POST /api/platform/invitations/confirm`. An expired or otherwise terminal organization envelope answers `409` `registration_conflict`; an expired or terminal Personal, invited-member or Platform envelope answers the collapsed `400` `invalid_confirmation`.
...
- **IA-REQ-054:** identity account state and tenant membership state are finite and explicit, and are the only source of the "active identity" condition IA-REQ-020 states. The identity states are `PendingConfirmation`, `Active`, `SelfDeactivated`, `AdministrativelySuspended` and the terminal `Closed`; only `Active` may sign in. Every transition is a conditional mutation guarded by an expected state, is audited, and commits with its session, token and outbox effects or commits none of them. Every non-terminal disabled state names an actor, a proof and an endpoint; `Closed` names none and says so. A person may park their own account and come back through a public, single-use, hash-compared reactivation ticket — 30 minutes **(product default)**, superseded on reissue — proving a current authenticator, which is a password; an identity that signs in only through a provider sets one first, because a provider round trip cannot demonstrate presence (see [section 14.6](#146-c6--finite-identity-and-membership-lifecycle-and-a-fail-closed-restore-admission-guard)). Reactivation restores the state that preceded the disable and never lifts an administrative suspension, which only a Platform operator lifts under a closed reason set recorded in audit alone. Retention is not lifecycle: a legal hold stops erasure and nothing else, is never an account state, refuses no sign-in and blocks no reactivation. Recovering a credential is not lifecycle either: a completed reset or provider recovery changes what a person can prove, never what their account may do, so it lifts no suspension and skips no second factor.
- **IA-REQ-055:** a restored deployment admits no public ingress, issues no session, accepts no restored session or one-time token and dispatches no outbox delivery until an operator-controlled admission record held outside the restored database verifies against an operator-held key supplied by environment configuration. Absent, unreadable, expired, wrongly signed, wrong-deployment or non-advancing evidence keeps admission closed on every process start, not only the first, and no value read from the restored database or contained in the backup can open it.
- **IA-REQ-053:** an `Organization` administers itself through custom roles, membership lifecycle and one explicit
  ...
  **Offers:** widening or retiring a role cancels in that transaction every pending invitation offering it and
  ...
  from anywhere but the validated session. `GetPermissionCatalog`, `ListRoles`, `GetRole`, `ListMembers`, and
  `ListTenantInvitations` treat a route tenant mismatch as native absence (`404 not_found`); role and membership
  writes retain their state-safe `400 invalid_role_operation` or `400 invalid_membership_operation` refusal. The
  route table, state table and contention rules are in
...
- **IA-REQ-038:** expected Domain/Application business failures use typed `Result`/`Result<T>` values with a stable code and category; unexpected infrastructure or programmer failures remain exceptions. Web maps both paths to the external HTTP contract and never serializes the internal Result or a universal `{ success, data, error }` envelope. Body-bearing success returns an endpoint-specific DTO with semantic status and headers (`200`; `201` with `Location`; the documented neutral `202`; or bodyless `204`). Every non-success is RFC 9457 `application/problem+json` with matching status, stable `code`, opaque `traceId`, optional safe `detail`, and field-indexed `errors` only for validation; a generic safe `500` exposes no internal diagnostics. A validation `errors` value is keyed by a camel-case JSON member path and every member is `{ "code": "snake_case", "params": { ... } }`: codes are repository-owned, stable vocabulary rather than FluentValidation type names, `params` is always present and contains only allowlisted non-sensitive numeric template metadata such as `max`, and neither framework messages, attempted values, tokens, addresses nor arbitrary placeholders cross the boundary. Generated `401`, `403`, and `429` use the same Problem Details writer, and `429` includes `Retry-After`. Sign-in, password-recovery initiation, and reactivation-request initiation are the explicit enumeration-sensitive binding exception: within the shared caller-attempt budget, malformed, wrong-typed, or absent JSON receives the same bodyless `204`/`202` as the route's neutral answer and creates no identity or business effect. Before that neutral answer, the request still applies exact-origin, invalid optional-session, and antiforgery checks in that order. The caller budget is deliberately spent before those checks and binding, persists independently of account or token state, and may therefore answer `429`/`503` before parsing. The exception applies only inside the positively identified request-binding window and only to parse/binding failures that would otherwise be `400`: a JSON- or bad-request-shaped exception after binding reaches application code remains an unexpected, safely logged generic `500`; an oversized request body keeps the existing `400` `invalid_request` Problem Details response and explicit declaration, while unsupported media keeps its existing empty `415`. OpenAPI declares each endpoint's success schema/status/required headers and supported error statuses/shapes/codes: a configured parameter-binding refusal declares its configured `400` code, every authorization boundary declares `401` `authentication_required` and `invalid_session` plus `403` `permission_denied`, every public optional-session rejection declares `401` `invalid_session`, and every shared login-attempt-budget marker declares both `429` `rate_limit_exceeded` and `503` `service_unavailable`. Contract tests reject drift across endpoint metadata, served OpenAPI, runtime refusals, and React. React consumes one typed API boundary, knows nothing of internal Result, and adds no pagination envelope to identity endpoints. That transport is the sole client failure classifier: it preserves a valid server Problem — including status, code, trace id, field errors, safe non-5xx detail, and `Retry-After` — and otherwise emits only the client-owned status-`0` codes `network_unavailable`, `request_timeout`, `unreadable_response`, or `client_failure`; those four codes never enter the server catalogue. Every request, including antiforgery bootstrap, has a 30-second transport deadline composed with an optional caller signal through `AbortSignal.any`: the deadline is `request_timeout`, a fetch `TypeError` is `network_unavailable`, an unreadable/non-Problem refusal or contract-drifting success is `unreadable_response`, and caller cancellation or any remaining client fault is `client_failure`. Effect cleanup suppresses its own cancellation rather than presenting stale state. A read is retryable exactly for status `0`, `429`, or `>=500`; a typed other `4xx` remains refused. Mutations are never automatically replayed, because a transport failure leaves their outcome unknown until the person refreshes; when an antiforgery refresh fails after a refused mutation, the original server Problem remains the answer. The shared renderer uses only local safe messages and adds selectable `Reference: <traceId>` text inside its single alert only for status `>=500`.

  **Offset paging (amended 2026-09-13; authorized by the repository owner's confirmation of PD-4 for
  `offset-pagination-standard`).** A paginated directory answers an endpoint-specific offset page DTO
  `{ items, pageNumber, pageSize, totalCount, totalPages, hasPreviousPage, hasNextPage }`. It is a declared success DTO,
  not an envelope, so React still adds no pagination envelope to identity endpoints. `pageNumber` (≥ 1) and `pageSize`
  (1–100, default 25) are optional and clamped, never refused, so a `pageSize` of 0 or less means 1. A non-integer or
  beyond-Int32 value is the route's `400 invalid_request`. A page past the end is `200` with empty `items`.

  The validation-detail sub-contract further requires every known correctable rule to use its specific code. The
  allowlist admits positive `max` and `min` metadata only where the schema names it; `invalid` is reserved for an
  unknown, malformed or unclassified detail, and `password_policy` for an unknown/custom Identity provider rule.
  Identity password failures are mapped from provider codes and live safe options, never provider descriptions, and
  multiple failures are preserved deterministically.

  Signed-in personal-context creation, personal-profile update, role create/update, and member-invitation issue use
  application validation for their documented request fields and return `400 validation_failed` with exact
  lower-camel field keys and structured value-free `{ code, params }` details from the repository-owned validation vocabulary. A profile member outside `fullName`, `displayName`, and
  `version` retains `400 profile_field_not_editable` naming only the deterministically selected member; malformed
  known fields are binding/validation failures, while a well-formed stale profile version remains the distinct
  `409 personal_profile_concurrency_conflict`. Authority, state, store, tenant, and role refusals retain their
  existing non-validation operation codes.

  Routing is part of IA-REQ-038: an unmatched `/api` request, including a failed route constraint such as a malformed `:guid`, returns `404` `not_found` through the shared Problem Details writer; `HEAD` retains that status and headers without an entity body. A known API path called with an unsupported method remains the ASP.NET routing-owned `405` with its `Allow` semantics and is not assigned a new problem code. An unmatched non-API `GET` or `HEAD` whose final segment is not file-like remains owned by the SPA fallback (`/apiary` is non-API and therefore belongs to that fallback); `HEAD` keeps its successful HTML headers without an entity body, while a file-like path such as `/missing.js` is not rewritten to the SPA index.

### Account language and localized identity delivery

> **Completed in Phase 4 and extended by Phase 5 on 2026-09-12.** IA-REQ-059 defines language preference and
> delivery-language behavior; IA-REQ-038 independently defines the structured validation-error representation.
...
- **IA-REQ-059:** an identity's language preference and every identity-access message delivered outside the SPA obey one explicit supported-language contract.
  ...
  - **Invariant and neutral surfaces.** Outbox payloads remain identifiers only, with no rendered prose, culture-dependent value, template argument or personal data. API and OpenAPI contracts, Problem Details, validation codes and template metadata, error and permission codes, routes, logs, audits, telemetry, developer exceptions, identifiers and URLs remain invariant English or machine data; the SPA translates validation codes from its catalog. Capturing or resolving language changes no status, body or timing distinction in registration, invitation, sign-in, recovery, reactivation or other enumeration-neutral public flows, and never reveals whether an account exists or has a preference.
...
| `POST /api/identity/organizations/register` | Public or authenticated + antiforgery | `400` `validation_failed` with camelCase field errors for missing, overlong, or malformed legal-name and CUIT input and, for an anonymous caller only, missing, overlong, or malformed request-only email and a missing, overlong, or PasswordOptions-rejected password; these refusals precede account/address/CUIT lookup, echo no submitted value, create no durable effects, and are byte-identical for taken and free state except for `traceId`/`instance`; every valid anonymous request keeps the neutral bodyless `202`, whatever the address and whatever the CUIT, reserving nothing; an authenticated request derives its address from the validated session, accepts a blank submitted email, ignores its unused password, returns `409` Problem Details `registration_conflict` for an already registered CUIT, and returns `400` `invalid_registration` when a nonblank submitted email does not normalize to the session's; a session the server cannot validate is `401` `invalid_session` |
| `POST /api/identity/personal/register` | Public + antiforgery, `IPublicRequest` | `400` `validation_failed` with camelCase field errors for missing, overlong, or malformed request-only email, full-name, display-name, and document-number input and for a missing, overlong, or PasswordOptions-rejected password; these refusals precede account/address/document lookup, echo no submitted value, create no durable effects, and are byte-identical for taken and free state except for `traceId`/`instance`; every valid anonymous request keeps the neutral bodyless `202`, whatever the address, reserving no documentary identity — the sealed document is held in the intent and no fingerprint row is written until the address is proved; a replay that finds the same canonical submission still in flight is `409` `personal_registration_conflict`; a valid request from an authenticated caller remains `400` `invalid_registration`, because that caller has a route that needs no mailed token; a session the server cannot validate is `401` `invalid_session`. Initiation spends no document-claim budget and therefore declares neither `429` nor `503`; that budget belongs to confirmation/finalization. |
| `POST /api/identity/personal` | Authenticated + `identity.profile.manage` (`RequiresTenant=false`) + antiforgery | `400` `validation_failed` with the same exact camelCase `fullName`, `displayName` and `documentNumber` rules and messages as Personal registration, decided before any durable identity, tenant, membership, document, audit or outbox effect; bodyless `204`; `409` `personal_registration_conflict` for every well-shaped reason the context cannot be recorded, indistinguishably; `429` `rate_limit_exceeded` + `Retry-After` for an exhausted claim budget; `503` `service_unavailable` + `Retry-After` when the shared budget store is unreachable. This route declares no `invalid_registration`. |
...
| `PUT /api/identity/profile` | Authenticated + `identity.profile.manage` (`RequiresTenant=false`) + antiforgery | `200` updated `PersonalProfileResponse`; blank/oversized `fullName` or `displayName`, and a missing or non-canonical unsigned-decimal `version`, are `400 validation_failed` with one exact camelCase field key and no submitted-value echo; a well-formed stale version remains `409 personal_profile_concurrency_conflict`; `400` `profile_field_not_editable` names only an unknown JSON member and never its value; `404` `personal_profile_not_found` |
| `POST /api/identity/confirm-email` | Public + token + antiforgery | bodyless `204` when the registration finalizes or already did; `409` Problem Details `registration_conflict` when an organization CUIT was taken first, its address gained an identity first, or its anonymous or pre-upgrade envelope expired or otherwise terminated — including on replay; a Personal finalization that cannot claim its documentary identity answers the collapsed `409` `personal_registration_conflict`; an exhausted Personal document-claim budget answers `429` `rate_limit_exceeded` + `Retry-After`, and an unreachable budget store answers `503` `service_unavailable` + `Retry-After`; `400` `invalid_confirmation` covers an unknown or malformed token and every expired or terminal Personal or invited-member envelope. The first clock expiry terminalizes the secret and also settles a Personal intent as `Expired`; neither purpose creates an identity graph or membership. |
| `POST /api/identity/sessions` | Public + antiforgery | bodyless `204` + cookie on success; within the shared caller-attempt budget, credential refusal and malformed, wrong-typed, or absent JSON all return the same bodyless `204`, with no session, cookie, audit, outbox, or other identity/business effect. The caller budget is spent before binding, persists independently of account state, and may return `429` `rate_limit_exceeded` or `503` `service_unavailable`; an oversized request body keeps the existing declared `400` `invalid_request` Problem Details response, and unsupported media keeps the existing empty `415`. `401` `credential_superseded` remains possible only when the credential this request validated was replaced before the session could be issued, which requires the correct password and a change only that identity could make |
...
| `POST /api/identity/credentials/password/recovery`; `POST /api/identity/credentials/password/reset` | Public + antiforgery; recovery rate-limited | within its shared caller-attempt budget, recovery answers the same neutral bodyless `202` whatever the address, account state, or malformed, wrong-typed, or absent JSON; an unbound body creates no password-reset request, outbox, audit, session, cookie, or other identity/business effect, while the caller budget is spent before binding, persists independently of account state, and may return `429`/`503`. An oversized recovery body keeps the existing emitted `400` `invalid_request` Problem Details response, now explicitly declared by P2-1, and unsupported media keeps the existing empty `415`. Reset answers bodyless `204`, `400` `invalid_credential_token` for unknown, expired, consumed, superseded or wrong-purpose, `400` `validation_failed` with structured `newPassword` details for a password the policy refuses, `409` `identity_concurrency_conflict` for a lost credential update, `429` `rate_limit_exceeded` when its per-identity lock cannot be acquired, or the generic safe `500` for an unexpected provider failure. Either route returns `401` `invalid_session` when an optional session cookie cannot be validated |
| `POST /api/identity/credentials/reauthenticate`; `PUT /api/identity/credentials/password` | Authenticated + antiforgery + `identity.credentials.manage`, `RequiresTenant=false`; the change additionally needs a live proof | bodyless `204`; `400` `invalid_credential_proof`; `401` `recent_proof_required`; the change also returns `400` `validation_failed` with structured `newPassword` details for a password the policy refuses or `409` `identity_concurrency_conflict` for a lost credential update, and answers success with a rotated session cookie and antiforgery pair |
...
| `POST /api/tenants/{tenantId}/invitations` | `members.invite` + antiforgery | `201` invitation DTO + `Location`; null, blank or malformed `email` is `400 validation_failed` with a sole safe structured detail under `errors.email`; null or empty `roleIds`, and a `Guid.Empty` member, use the same envelope with a sole repository-owned `{ code, params }` detail under `errors.roleIds`. No framework message or attempted value crosses the boundary, and these field refusals write no invitation, outbox secret/message or audit event. A route-tenant mismatch retains `400 invalid_invitation`; this route does not declare `404 not_found`. |
...
| `POST /api/invitations/register` | Public + invitation credential-registration DTO + antiforgery | `400` `validation_failed` with `errors.token` for a missing or overlong token and `errors.password` for a missing, overlong, or PasswordOptions-rejected password; these request-only refusals are decided before token/account lookup and are byte-identical for live and dead tokens except for `traceId`/`instance`; every well-shaped, PasswordOptions-valid request remains a neutral bodyless `202` regardless of token or account state; registration/confirmation only |
...
| `POST /api/platform/invitations/register` | public + Platform invitation credential-registration DTO + antiforgery | `400` `validation_failed` with `errors.token` for a missing or overlong token and `errors.password` for a missing, overlong, or PasswordOptions-rejected password; these request-only refusals are decided before token/account lookup and are byte-identical for live and dead tokens except for `traceId`/`instance`; every well-shaped, PasswordOptions-valid request remains a neutral bodyless `202` regardless of token or account state; a missing identity uses the submitted password and an existing identity ignores it; issue confirmation, never membership |
| `POST /api/platform/invitations/confirm` | public + one `confirmationToken` + antiforgery; its sealed envelope identifies the invitation and bound recipient | bodyless `204` when confirmation consumes the envelope and on replay of that consumed success; `400` `invalid_confirmation` for an unknown, malformed, wrong-purpose, expired or otherwise terminal envelope, a binding mismatch, or an invitation that is no longer pending; every refusal confirms no identity and creates no membership or onboarding audit effect |
| `POST /api/platform/mfa/enroll`, `/verify`, and `/recovery-acknowledge` | authenticated, confirmed pending Platform invitee whose normalized email matches its bound one-time invitation token + antiforgery; no active Platform tenant is required before activation | enrollment DTO, then bodyless `204`; after that admission succeeds, `/verify` answers `400` `invalid_mfa_code` for a missing enrollment or rejected authenticator code without distinguishing them, `429` `rate_limit_exceeded` + `Retry-After` once the identity's shared attempt budget is exhausted, and `503` `service_unavailable` + `Retry-After` when that budget store is unreachable |
| `POST /api/platform/mfa/step-up` | authenticated Platform identity + antiforgery; no active Platform tenant is required so a password-only session can prove the factor | bodyless `204`; after a valid session is admitted, `400` `invalid_mfa_code` for no enrollment, unfinished enrollment, or a rejected authenticator code without distinguishing them; genuine session failures remain `401` `invalid_session`; `429` `rate_limit_exceeded` + `Retry-After` and `503` `service_unavailable` + `Retry-After` use the same per-identity budget as `/verify` |
| `POST /api/platform/mfa/recover` | authenticated confirmed identity + antiforgery + fresh primary proof; no active Platform tenant is required | `200` enrollment DTO shown once; after proof, `400` `invalid_recovery_code` for no enrollment, unfinished enrollment, or a wrong, spent, or stale recovery code without distinguishing them; genuine session failures remain `401` `invalid_session`, missing proof is `401` `recent_proof_required`, concurrent replacement is `409` `platform_mfa_concurrency_conflict`, and `429`/`503` use the same per-identity budget as `/verify` |
| `GET /api/platform/organizations` with `pageNumber`/`pageSize` | active Platform tenant + `platform.organizations.read` + this session has proved the second factor | `200` typed `{ items: PlatformOrganizationResponse[], pageNumber, pageSize, totalCount, totalPages, hasPreviousPage, hasNextPage }`, or `401` Problem Details `recent_mfa_required` |
| `GET /api/platform/identities` with `pageNumber`/`pageSize` | active Platform tenant + `platform.identities.read` + this session has proved the second factor | `200` typed `{ items: PlatformIdentityResponse[], pageNumber, pageSize, totalCount, totalPages, hasPreviousPage, hasNextPage }`, or `401` Problem Details `recent_mfa_required` |
| `GET /api/platform/admins` with `pageNumber`/`pageSize` | active Platform tenant + `platform.admins.read` | `200` bounded `{ items, pageNumber, pageSize, totalCount, totalPages, hasPreviousPage, hasNextPage }` administrator directory DTO |
| `GET /api/platform/audit` with `pageNumber`/`pageSize` | active Platform tenant + `platform.audit.read` | `200` typed `{ items: PlatformAuditEventResponse[], pageNumber, pageSize, totalCount, totalPages, hasPreviousPage, hasNextPage }` |
...
The client uses `permissions` only for UX. It always handles status-`0` client failures and server `401`, `403`, `404`, `409`, `429`, and `5xx` answers because the backend reauthorizes every operation. Active tenant state is never derived from React state or a client-supplied header.

## 8. Initial web security contract

- Same-origin BFF; no open CORS and no bearer token for the SPA.
- `GET /api/identity/antiforgery` creates the `Secure`, `HttpOnly`, `SameSite=Lax`, `Path=/`, host-only cookie `__Host-XSRF-TOKEN` and returns `{ "requestToken": "..." }` with `Cache-Control: no-store`. React keeps that request token in memory only and sends it as `X-CSRF-TOKEN` on every state-changing request, including public credential or invitation submissions. Bootstrap clears any cached request token before it fetches, uses the same 30-second deadline and caller-signal classifier as every other request, and leaves the token absent after a failed refresh so the next mutation bootstraps lazily.
- The server rotates the antiforgery cookie/request-token pair whenever authentication state or the session identifier changes, including successful sign-in and sign-out. The client fetches a fresh pair after initial load, page reload, sign-in, sign-out, or a stable `antiforgery_validation_failed` response; old pairs are rejected after rotation. A successful sign-in is not reported as failed merely because its immediate token refresh fails: the client still reloads identity context so the existing session is reflected, and the next mutation performs the lazy bootstrap. Neither token is logged or persisted by the client.
...
Scenario: Platform directories are bounded operational projections
  ...
  When the administrator requests `/api/platform/admins` with a bounded `pageNumber` and `pageSize`
  Then the typed directory returns only its declared allowlisted fields and the offset page metadata
...
> **EVERY ENTRY C1–C7 IS NOW ACCEPTED — C1–C5 and C7 on 2026-09-06, C6 on 2026-09-07 with one withdrawal (E1).**
> All seven subsections are accepted and their requirements now live in section 4; those subsections are kept as the
> record of the decisions that produced them, and section 4 governs where the wording differs. Every acceptance is
> **for implementation and verification against synthetic data only**: real personal data and production deployment
> are two separate gates, and neither is granted by any of them. **C5 was accepted with
...
- **IA-REQ-003 (accepted amended wording; section 4 governs):** organization registration deterministically follows the caller
  ...
  - An authenticated request creates another organization for the current identity. Its address comes from the
    validated session; a blank submitted email is treated as absent, any nonblank submitted email must normalize
    to the session address, the unused password is ignored, an unvalidatable session is rejected as invalid, and
    an already registered CUIT creates nothing, returning `409` `registration_conflict`.
...
| Method and route | Access | Primary result |
|---|---|---|
| `POST /api/identity/organizations/register` | Public or authenticated + antiforgery; `IPublicRequest` + `ISensitiveRequest` | neutral bodyless `202` for every anonymous request, whatever the address and whatever the CUIT, reserving nothing; an authenticated request derives its address from the validated session, accepts a blank submitted email and ignores its unused password; an already registered CUIT is `409` Problem Details `registration_conflict`; a nonblank submitted email that does not normalize to the session's is `400` `invalid_registration`; a session the server cannot validate is `401` `invalid_session` |
| `POST /api/identity/confirm-email` | Public + intent token from the link fragment + antiforgery; `IPublicRequest` + `ISensitiveRequest` | bodyless `204` when an intent finalizes, when a pre-upgrade organization confirmation activates its identity/tenant/membership, or when either already did; `409` Problem Details `registration_conflict` when an organization CUIT was taken first, its address gained an identity first, or its anonymous or pre-upgrade envelope expired or otherwise terminated; a Personal intent that loses its documentary claim answers the collapsed `409` `personal_registration_conflict`, while its exhausted/unavailable claim budget answers `429` `rate_limit_exceeded` / `503` `service_unavailable`, each with `Retry-After`; `400` `invalid_confirmation` for an unknown or malformed token and for an expired or terminal Personal or invited-member envelope, with first clock expiry settling a Personal intent as `Expired` and neither purpose creating an identity graph or membership |
...
- **Amends.** IA-REQ-003 loses its first bullet, generalizes its second and is **contradicted** in its third, every
  ...
  IA-REQ-005 gains the anonymous-organization confirmation message type and its `409` answer on `confirm-email`.
...
| `PUT /api/identity/profile` | Authenticated + `identity.profile.manage` (application-scoped, `RequiresTenant=false`) + antiforgery + exact origin; `UpdatePersonalProfileRequest` accepts exactly `fullName` (1–200 characters, **product default**), `displayName` (1–60, **product default**) and `version` | `200` updated `PersonalProfileResponse`; `400` `validation_failed` with field-indexed `errors`, including a `version` this server never issued; `400` `profile_field_not_editable` naming the rejected member and never its value, mapped deliberately by Web from the request DTO's `JsonExtensionData` dictionary by selecting the first member name in ordinal order without inspecting, logging or echoing its value; `400` `antiforgery_validation_failed`; `401` `invalid_session`; `404` `personal_profile_not_found`; `409` `personal_profile_concurrency_conflict` |
...
| `POST /api/identity/credentials/password/recovery`; `POST /api/identity/credentials/password/reset` | both public + antiforgery, `IPublicRequest`; recovery rate-limited with `RequestPasswordRecoveryCommand { email }`, reset carrying the fragment token with `ResetPasswordCommand { token, newPassword }`, `ISensitiveRequest` | recovery: within the shared caller-attempt budget, neutral bodyless `202` for every valid request and every account state and also for malformed, wrong-typed, or absent JSON; an unbound body creates no password-reset request, outbox message, audit, session, cookie, or other identity/business effect. The caller budget is spent before binding and persists independently of account state; it may return `429` `rate_limit_exceeded` + `Retry-After` and, when the shared store is unreachable, `503` `service_unavailable` + `Retry-After` (IA-REQ-057). An oversized body keeps the existing emitted `400` `invalid_request` Problem Details response, now explicitly declared by P2-1, and unsupported media keeps the existing empty `415`; antiforgery failure remains `400` `antiforgery_validation_failed`. What is enqueued behind the neutral answer is fixed by account state and is stated once, here and in 14.6: `PendingConfirmation`, `Active`, `SelfDeactivated` and `AdministrativelySuspended` each enqueue a reset token; `Closed` enqueues nothing. Reset: bodyless `204`, `400` `invalid_credential_token` for unknown, expired, consumed, superseded or wrong-purpose, `400` `validation_failed` with safe structured `{ code, params }` details under `newPassword` for `PasswordOptions`, `409` `identity_concurrency_conflict` when the credential row changed or disappeared, or `429` `rate_limit_exceeded` + `Retry-After` when its per-identity lock cannot be acquired. Either route returns `401` `invalid_session` when an optional session cookie cannot be validated. An unexpected provider failure uses the generic safe `500` path and exposes no provider description. A completed reset changes the password, increments the security version, revokes every session and issues none — and it changes no lifecycle state, so a suspended account is still suspended and a self-deactivated one is still deactivated; it never sets `LastVerifiedAt`, never marks a session as having proved a second factor and never removes a `PlatformMfaEnrollment`, so the next sign-in meets exactly the gates it met before |
...
| Active `Organization`, permission held, identity unconfirmed | `400` | `400` | `400` | `400` | unchanged (IA-REQ-005/014) | `400` |
| Active `Organization`, permission held, route `tenantId` differs from the session's active tenant | `404` `not_found` | `400` `invalid_role_operation` | `404` `not_found` | `400` `invalid_membership_operation` | `400` `invalid_invitation` | `400` `invalid_membership_operation` |
| Active `Organization`, permission held, actor not the owner | n/a | n/a | n/a | n/a | n/a | `403` `owner_required` |

| Method and route | Access | Primary result |
|---|---|---|
| `GET /api/tenants/{tenantId}/permission-catalog`; `GET .../roles` with `pageNumber` (≥ 1) / `pageSize` (1–100); `GET .../roles/{roleId}` | `roles.read` | `200` `PermissionCatalogEntryResponse[] { code, grantable }`; `200` typed `{ items: RoleResponse[], pageNumber, pageSize, totalCount, totalPages, hasPreviousPage, hasNextPage }`; `200` `RoleResponse { roleId, name, isSystem, isRetired, permissions, version }`, another tenant's role being `404` (IA-REQ-030) |
| `POST /api/tenants/{tenantId}/roles` | `roles.manage` + antiforgery; `CreateRoleRequest { name, permissions }`, with `name` required and at most 128 characters | `201` `RoleResponse` + `Location`; blank `name` is `400 validation_failed` with the sole `errors.name = [{ "code": "required", "params": {} }]`, and an over-128 name is the same envelope with the sole `errors.name = [{ "code": "too_long", "params": { "max": 128 } }]`; ceiling refusal `400` `invalid_role_operation`; duplicate normalized name `409` `role_concurrency_conflict`. This route does not declare `last_administrator_required`, because creating a role cannot lower the administrator floor. |
| `POST /api/tenants/{tenantId}/roles/{roleId}/retire` | `roles.manage` + antiforgery; `RetireRoleRequest { version }` | bodyless `204`, already retired with a current `version` being an idempotent `204`; state or authority refusal `400` `invalid_role_operation`; stale `version` `409` `role_concurrency_conflict`; `409` `last_administrator_required` |
| `PUT /api/tenants/{tenantId}/roles/{roleId}` | `roles.manage` + antiforgery; `UpdateRoleRequest { name, permissions, version }`, with `name` required and at most 128 characters | `200` `RoleResponse`; blank or over-128 `name` has the same exact `400 validation_failed` `errors.name` contract as create; ceiling, system-role or retired-role refusal `400` `invalid_role_operation`; stale `version` `409` `role_concurrency_conflict`; removing the last effective administrator `409` `last_administrator_required` |
| `GET /api/tenants/{tenantId}/members`; `GET .../invitations`, both with `pageNumber` (≥ 1) / `pageSize` (1–100) | `members.read` | `200` typed `{ items: MemberResponse[], pageNumber, pageSize, totalCount, totalPages, hasPreviousPage, hasNextPage }` with `MemberResponse { membershipId, identityId, displayName, normalizedEmail, status, roleIds, isOwner, version }`; `200` typed `{ items: InvitationSummaryResponse[], pageNumber, pageSize, totalCount, totalPages, hasPreviousPage, hasNextPage }` with `InvitationSummaryResponse { invitationId, normalizedEmail, status, createdAt, expiresAt, roleIds }` |
...
- **Amends.** Replaces IA-REQ-047's closing deferred-control clause: a widened role no longer reaches acceptance, the
  ...
  **C5 history note (2026-09-13):** the sentence above stays as the record of what C5 amended. Section 4's offset contract
  in IA-REQ-038 and IA-REQ-045 supersedes that shape for the tenant and Platform directory reads
  (`offset-pagination-standard`, PD-4).
...
- **As built, where the implementation and this record differ.** Recorded because the decision governs and the
  ...
  - **The exposed version moves when the relationships move (2026-09-07).** A role's `version` and a membership's
    `version` are their own rows' `xmin`, and PostgreSQL advances that only when the row itself is rewritten — so
    an edit that changed only a permission set or only a role assignment left the version every administrator was
    editing from untouched, and a second edit echoing it was applied over the first instead of refused. Both write
    paths now rewrite the owning row in the same transaction. That restores the declared answer
    (`role_concurrency_conflict` / `membership_concurrency_conflict`) and it also puts the token back into the
    UPDATE's own `WHERE`, so a racer is refused by the database rather than by a comparison. The interceptor now
    records one more `role.changed` / `membership.changed` row, with a null actor, for these edits: the role and
    the membership did change, and under-reporting that was the previous behaviour rather than a decision.
  - **A revoked member returns through a fresh invitation (2026-09-07).** C5 allows `Revoked → Active` only that
    way, but the issue path refused an offer to anybody holding a membership row of any status, and acceptance
    refused for the same reason — so a removed person could never come back. A `Revoked` row is now read as the
    record of a removal rather than a place in the organization: it does not block a new offer, and accepting one
    reinstates that same membership rather than inserting a second. It comes back holding only what the new offer
    names; revocation had already deleted every assignment it held.
  - **An offer is authorized at commit time, not only at validation time (2026-09-07).** Cancelling the offers of
    a widened role can only reach offers that are committed, and an offer validated but not yet inserted is
    invisible to it — so the two could each commit against a world in which the other had not happened, and the
    recipient ended up holding a permission the inviter never held. A per-tenant advisory lock (a fourth space,
    `0x5E5512`) now orders them: a role change takes it first, before it holds anything, and waits; an offer takes
    it last, after its rows are written, and never waits. Whoever cannot take it re-reads and refuses with the
    already-declared `invitation_conflict`.
- **Consumed by** Tasks 24 and 25.

### 14.6 C6 — Finite identity and membership lifecycle, and a fail-closed restore admission guard

> **ACCEPTED 2026-09-07, for synthetic data only, with one withdrawal.** IA-REQ-054 and IA-REQ-055 are now
> normative and live in [section 4](#4-normative-requirements); this subsection is kept as the record that produced
> them. The acceptance enables **Tasks 26 and 27** and the production gate in Task 28, and nothing after 28.
>
> **E1 — the provider reactivation half is withdrawn, and with it C4's `Recovery` purpose.** Google documents that
> it does not support Google Account reauth requests; `auth_time` is returned only when requested through the
> `claims` parameter and enabled in the project's security-bundle settings; and `max_age` is not among the
> authorization parameters Sign in with Google documents. A provider round trip can therefore show that the browser
> still holds a live session at the provider — not that a person is present now. Treating that as a "current
> authenticator" would mean trusting a token whose issue time proves nothing about authentication, or relaxing the
> freshness rule C4 already enforces; neither is taken. **Decided instead:** a provider-only identity that parked
> its own account sets a password first and reactivates with it. Both steps exist already, both are gated on control
> of the same mailbox the ticket is sent to, and password recovery still changes nothing about lifecycle.
> `ExternalAuthorizationPurpose.Recovery` is not implemented and `providerProofToken` leaves the reactivation
> request. Where this banner and the drafted text below differ, this banner governs.
>
> **Amended 2026-09-06 (A3, A4).** The provider-only return path is now defined
...
| `POST /api/identity/account/reactivation-requests`; `POST /api/identity/account/reactivate` | both `IPublicRequest` + antiforgery + rate limit; `RequestAccountReactivationRequest { email }`; `ReactivateAccountRequest { reactivationToken, password?, providerProofToken? }`, exactly one of the last two, the token single-use, hash-compared, 30 minutes **(product default)**, superseded on reissue; `providerProofToken` is the single-use handle returned by completing a C4 `Recovery` handoff started from this same reactivation ticket, which is how an identity that only ever signs in through a provider proves a current authenticator without the session it is being denied | within the shared caller-attempt budget, the request route returns the same neutral bodyless `202` for every valid request and every account state and for malformed, wrong-typed, or absent JSON; an unbound body creates no reactivation request, outbox message, audit, session, cookie, or other identity/business effect, and only a valid request for a `SelfDeactivated` identity enqueues anything. The caller budget is spent before binding and persists independently of account state; it may return `429` `rate_limit_exceeded` + `Retry-After` and, when the shared store is unreachable, `503` `service_unavailable` + `Retry-After` (IA-REQ-057). An oversized request body keeps the existing emitted `400` `invalid_request` Problem Details response, now explicitly declared by P2-1, and unsupported media keeps the existing empty `415`. Reactivation then answers bodyless `204` with no session and no cookie, restoring the pre-disable state and never lifting an administrative suspension — a suspended or `Closed` identity has no self-service way back and this route does not become one; `400` `invalid_reactivation` for a bad, spent, expired or terminal-state token and for a failed proof, worded identically; `400` `antiforgery_validation_failed`; either route returns `401` `invalid_session` when an optional session cookie cannot be validated |
