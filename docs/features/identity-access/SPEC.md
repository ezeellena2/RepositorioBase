...
- **IA-REQ-047:** an invitation may offer only roles whose permissions the inviter already holds in that tenant. A role granting any permission outside the inviter's own effective set is refused when the invitation is issued and when it is reissued, so `members.invite` never grants more authority than the inviter has. Acceptance grants exactly the roles that were offered; a role whose permissions widen after the offer was made is out of scope for the first increment and is recorded as a deferred control rather than claimed.
