...
| `POST /api/identity/credentials/password/recovery`; `POST /api/identity/credentials/password/reset` | Public + antiforgery; recovery rate-limited | neutral bodyless `202` whatever the address and whatever the account state; reset answers bodyless `204`, `400` `invalid_credential_token` for unknown, expired, consumed, superseded or wrong-purpose, `400` `validation_failed` for a password the policy refuses, `409` `identity_concurrency_conflict` for a lost credential update, or `429` `rate_limit_exceeded` when its identity lock cannot be acquired |
| `POST /api/identity/credentials/reauthenticate`; `PUT /api/identity/credentials/password` | Authenticated + antiforgery + `identity.credentials.manage`, `RequiresTenant=false`; the change additionally needs a live proof | bodyless `204`; `400` `invalid_credential_proof`; `401` `recent_proof_required`; the change also returns `400` `validation_failed` for a password the policy refuses or `409` `identity_concurrency_conflict` for a lost credential update, and answers success with a rotated session cookie and antiforgery pair |
