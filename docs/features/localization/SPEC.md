...
promotion and enforcement gates. No translation platform is adopted at the current scale, and no third language is
added until the business requests one.
...
The requirements are normative. Evidence cells reflect the current integration state: Phases 0–5 and the
actionable-validation follow-up are delivered. Phase 6 implementation and independent review are complete; the full
verification rerun on the integrated current-`main` tree and delivery remain pending.
...
| **L10N-REQ-013** — Logs, audit records, outbox payloads, codes, identifiers, URLs, and routes MUST remain invariant English or machine data and MUST NOT contain localized display text. | Invariant-data contract | All | Delivered through Phase 5 and the actionable-validation follow-up, including validation-boundary and provider-description non-disclosure tests; Phase 6 preserves the boundary locally, with integrated verification pending. |
