...
query `?lng=en-XA` is present. It is never a supported or in-progress language, catalog directory, MUI mapping,
journey row, cookie, account preference, backend value, or database value.
...
npx vitest run src/i18n/catalog.contract.test.js src/i18n/pseudoLocale.test.js src/i18n/spanish.test.jsx
npm run i18n:unused
npm run lint
cd ../../..
