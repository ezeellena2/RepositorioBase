...
| # | Question | Recommendation | Why |
|---|---|---|---|
| P5.1 | Code vocabulary | Our own stable snake_case rule codes: shared rules (`required`, `too_long`, `unsupported_value`), actionable email/CUIT/DNI/role/version rules, and granular password-policy rules; `password_policy` is reserved for unknown/custom provider rules and `invalid` for unknown, malformed or unclassified details | The same style as `ApplicationError.Code`, independent of the library; every known correctable rule stays actionable while fallback codes remain safe |
...
2. **Complete:** every current FluentValidation component declares `WithErrorCode(...)`; the server-owned schema
   permits `too_long` only with exactly one positive integer `max`, permits the two configurable password rules only
   with exactly one positive integer `min`, normalizes malformed details to safe `invalid`, strips parameters from
   no-parameter codes, and `ProblemDetailsExceptionHandler` writes immutable `{ code, params }`.
3. **Complete:** SPA field errors render every detail from `errors:validation.<code>` plus projected safe params;
   known `required` fields use a bounded field-specific catalog message. Unknown or adversarial codes use the
   translated generic fallback without changing locale. Form-owned controls associate and focus their inline refusal
   without duplicating it globally; only summary errors owned by a rendered control receive a translated field label
   and focus link, while request-only, hidden and otherwise unowned details render without a fabricated field prefix.
