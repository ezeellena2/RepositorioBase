...
**State (2026-09-07): complete.** The package was written into SPEC §14 and ADR-004, and every entry C1–C7 is now accepted for synthetic data only — C1/C3/C7, then C2/C4, then C5 with D1–D4 on 2026-09-06, and C6 with A3/A4 and withdrawal E1 on 2026-09-07. E1 withdraws the provider reactivation half and C4's `Recovery` purpose: Google documents that it does not support reauth requests, so a provider round trip cannot demonstrate that a person is present, and a provider-only identity sets a password before reactivating.
...
- [ ] **Step 1 — draft the exact contract delta.** For C1–C7, add caller/state tables, allowed DTOs, endpoint methods/routes/statuses/codes, permission/public markers, invariants, transitions, concurrency winners, audit/outbox effects, and failure examples to SPEC/ADR. Reuse existing permission constants where accurate; explicitly propose new self-profile/credential/ownership/lifecycle codes and their scope. Record accepted numerical settings and environment prerequisites, not unspecified configurable behavior.
  C6 must explicitly name the reactivation actor, fresh proof available before ordinary session issuance, and the distinction between self-deactivation and an administrative suspension. **Answered on 2026-09-07:** the reactivation actor is the person, the authority is a public single-use emailed ticket plus a current password, and self-deactivation is the only disabled state that route reaches — an administrative suspension is lifted by a Platform operator alone. An identity that signs in only through a provider sets a password first, because withdrawal E1 establishes that a provider round trip cannot demonstrate presence.
...
- [x] **RED:** disable then exercise password/Google/cookie/tenant operations; none can bypass disabled state. Recover MFA with wrong/reused code, stale or other-session proof, and concurrent consumption; only one replacement factor becomes usable after acknowledgement. Revocation, security version, factor/token state, audits and applicable notifications roll back together on injected real-write failure.
- [x] **GREEN lifecycle:** implement accepted disable/reactivation and factor-replacement transitions; rotate/retire old factor material and revoke the required sessions/proofs. Preserve at least one viable Platform owner/recovery path without introducing a default credential or automatic elevation. Expose only the approved recovery UI; recovery material is shown once and never logged/stored by React.
- [x] **RED/GREEN retention:** with synthetic data and `TimeProvider`, test bounded batches, stable cursor/CAS, expired vs live records, legal hold, unknown policy, replay, cancellation and crash/resume. Terminalize secrets safely; preserve required audit/delivery evidence. Purge only approved data categories after policy eligibility, retaining documented uniqueness or erasure evidence as required. Missing policy means no destructive action. Real-data purge is outside local tests.
- [x] **RED/GREEN dispute:** open a dispute with a stale, absent or other-session proof and with one already open; resolve one as the subject's own operator identity, without a stored dispute, and with an evidence reference outside the accepted shape. Prove a `corrected` resolution replaces ciphertext and every retained fingerprint row in one transaction or none, that it is refused when the claimed tuple belongs to another identity, that no claimed or recorded value reaches a log, audit record, outbox payload or response, and that a dispute changes nothing about sign-in, the Personal tenant or the recorded value until it resolves.
- [x] **RED/GREEN restore (reopened and re-done 2026-09-07; `RestoreRehearsalTests` takes a real `CREATE DATABASE … TEMPLATE` copy. The clause about external deletion evidence is **not** met and is recorded as open — nothing models it yet):** use an isolated synthetic backup that predates session revocation, password/link/factor replacement, grant removal and documentary purge. Start with ingress/delivery closed. The admission adapter checks the accepted operator-controlled authority outside that restored database; missing/stale/invalid evidence fails closed on every restart. Quarantine restored credentials/grants, refuse old cookies/tokens and tokenized messages, and require approved reproof/reconciliation before explicit release. Insufficient external deletion evidence keeps Personal data quarantined. A flag restored from the same backup or stamp rotation alone cannot pass the test.
- [x] **REFACTOR/verify (2026-09-07):** separate eligibility policy, bounded executor and admission adapter; run the commands below. Expected runtime unsafe-resurrection/replay/retention RED, then safe transitions and persistent closed admission GREEN. Record required external authority and remaining deviations in the runbook; synthetic adapter proof is not live restore certification.

```powershell
dotnet test tests/Application.FunctionalTests/Application.FunctionalTests.csproj --filter "IdentityLifecycleTests|PlatformMfaRecoveryTests|DocumentDisputeTests|RecoveryAdmissionTests|RetentionHoldRaceTests|RestoreRehearsalTests"
dotnet test tests/Infrastructure.IntegrationTests/Infrastructure.IntegrationTests.csproj --filter "RetentionLifecycleTests|RestoreAdmissionTests|OutboxAdmissionTests|MigrationUpgradeTests"
npm test --prefix src/Web/ClientApp -- MfaRecoveryPage.test.jsx
```

- [x] **Done 2026-09-07, after a reopening.** Four gaps were found immediately after the first closure — two unwired ports, one unserialized race, and a restore rehearsal that had never been run — and all four are closed with a failing test, a fix and a vacuity check each; see [TASKS.md](../../features/identity-access/TASKS.md#task-26---closed-again-2026-09-07-after-being-reopened). The accepted local lifecycle and recovery contract is implemented and tested; maintenance cannot outrun policy and restore remains closed until valid evidence. Real-PII purge, external recovery-authority provisioning and live restore release are recorded as separate blocked operations until approved/proved, not declared optional or completed by these tests.

**Delivered as six reviewable units**, each with its own dated entry in
[TASKS.md](../../features/identity-access/TASKS.md) and its own RED, GREEN and vacuity check: 26.1 identity
lifecycle, 26.2 administrative suspension, 26.3 Platform MFA recovery, 26.4a the retention policy and legal holds,
26.4b the bounded executor, 26.5 the documentary dispute, 26.6 the restore admission guard. The runbook the file
list asks for is [RECOVERY-AND-RETENTION.md](../../features/identity-access/RECOVERY-AND-RETENTION.md).

**Three deviations from the drafted contract, each recorded where it was taken rather than silently absorbed.**
The MFA recovery proof is a server-side C4 row rather than a request `proofToken`, because that is what C4's
proofs are. "No active Platform tenant" on that route is an absence from its requirement list rather than a
prohibition, because signing in selects the only tenant an operator belongs to and reading it as a prohibition
made the route unreachable for exactly the person it exists for. And the restore guard is armed by naming a
deployment rather than by default, because closing every deployment by default would make an ordinary start
indistinguishable from a restore.

**Carried into Task 27, named rather than dropped:** `503` `service_unavailable` when a shared budget store is
unreachable, on every route that names a budget; the quarantine state's reads of restored sessions and
`OutboxSecret` rows; and a startup refusal when the restore guard is armed without a key.
...
- [x] **RED shared limits (2026-09-07):** exhaust each applicable budget across two independently constructed service instances using one PostgreSQL store; restart one instance and show the budget remains spent. Include parallel requests at threshold, account spelling variants, independent IP/account limits, MFA new-session evasion, bootstrap/recovery replay, expiry and `Retry-After`. Set accepted numeric budgets explicitly; do not count per-instance memory as distributed proof. Task 19 landed the store and proved only that it persists a spent window; **every scope gets its cross-instance and restart evidence here**, `personal.document.claim` included, and the scopes arriving from process-local limiters additionally have to be moved onto the shared store before they can be proved at all.
- [x] **GREEN (2026-09-07):** move the remaining budgets onto the adapter Task 19 landed, and add bounded expiry cleanup; retain deterministic time tests and separate credential lockout from transport limits. Use existing PostgreSQL as the proposed deployment default after C7 approval; add neither Redis nor a cloud provider merely for this task. Include storage-unavailable and high-cardinality-abuse behavior with a documented fail-closed policy and safe telemetry.
- [x] **RED/GREEN keys and transport (2026-09-07):** construct independent Web/worker Data Protection providers over a temporary shared repository and wrapping certificate, encrypt in one/decrypt in the other, rotate/restart and repeat. Reject wrong discriminator/certificate/purpose and an unreadable envelope. Preserve existing Resend and explicit local-folder modes. Test exact-origin enforcement behind trusted/untrusted forwarding, CSP/security headers and absence of callback codes, session handles, documents, addresses and secrets in diagnostics.
- [x] **REFACTOR/verify operations (2026-09-07):** record bounded metrics/alerts for login/recovery denials, unreadable/expired envelopes, backlog age, delivery failures and maintenance/restore admission. Exercise loss of key/transport/state using isolated sinks; backlog recovery must respect original token expiry/idempotency, not resend a superseded credential. The operations guide separates synthetic smoke, operator configuration, external-account activation, and release evidence.

```powershell
dotnet test tests/Infrastructure.IntegrationTests/Infrastructure.IntegrationTests.csproj --filter "SharedAttemptBudgetTests|WebWorkerKeyCompatibilityTests|AttemptBudgetCleanupTests|IdentityAccessMetricsTests|EmailConfigurationTests|OutboxDeliveryTests|MigrationUpgradeTests"
dotnet test tests/Application.FunctionalTests/Application.FunctionalTests.csproj --filter "IdentityDeploymentGuardTests|ForwardedHeadersSecurityTests|SharedAbuseControlTests|SafeTelemetryTests|PlatformMfaAttemptLimitTests|PlatformBootstrapRecoveryTests"
```

- [x] **Done 2026-09-07.** Runtime tests prove the budgets across independently constructed instances and across the disposal of the one that spent them, and prove cross-process key compatibility by sealing in one provider and opening in another; configuration was checked nowhere and would not have been sufficient. Every process-local limiter is gone: `TimeProviderFixedWindowRateLimiter`, `PlatformMfaAttemptLimiter` and the bootstrap recovery dictionary are replaced by rows, and `IPlatformMfaAttemptLimiter` went with its only implementation. Deployment evidence is [OPERATIONS.md](../../features/identity-access/OPERATIONS.md), which names the key, PII and recovery owners and the still-blocked external prerequisites in a section of their own. No production, complete-standard or legal-compliance claim follows from any of it.

**One expectation was wrong and is recorded rather than dropped.** Data Protection does not refuse a key ring it cannot decrypt: it skips the keys it cannot read and writes one of its own. A deployment given the wrong wrapping certificate therefore looks healthy — new mail is sealed and sent — while every envelope already in flight is dead. Nothing warns, which is why `identity_access.outbox.settlements` carries a `reason` label and why OPERATIONS names `envelope_unreadable` as an alert.

**What this task does not claim.** Two hosts in one test process share a database and nothing else, which is the property under test — but it is not a network, and no genuinely multi-machine deployment has been exercised.
