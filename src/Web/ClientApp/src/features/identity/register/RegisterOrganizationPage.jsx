    ...
    <Paper component="section" elevation={3} aria-labelledby="register-heading" sx={card}>
      <Stack spacing={3}>
            ...
            {!signedIn && (
              <Stack spacing={1}>
                <Typography variant="body2">
                  <Trans
                    i18nKey="identity:register.organization.existingAccount"
                    components={{ signIn: <Link component={RouterLink} to={signInPath} /> }}
                  />
                </Typography>
                <Typography variant="body2">
                  {t('register.organization.deliveryHint')}
                </Typography>
              </Stack>
            )}
          </Stack>
        ...
        <Stack component="form" spacing={3} noValidate onSubmit={submitForm}>
          ...
          <Button type="submit" variant="contained" size="large" fullWidth disabled={isBusy}>{t('register.organization.submit')}</Button>
        </Stack>
        ))}
      </Stack>
    </Paper>
