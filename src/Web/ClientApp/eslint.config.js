...
const structuralJsxAttributeExclusions = [
  'className', 'styleName', 'style', 'type', 'key', 'id', 'width', 'height',
  'component', 'to', 'href', 'sx', 'variant', 'color', 'position', 'edge', 'size', 'severity',
  'autoComplete', 'inputMode', 'maxWidth', 'direction', 'role', 'scope', 'align', 'placement',
  'timeout', 'headingId', 'data-testid', 'aria-labelledby', 'aria-describedby', 'aria-controls',
  'aria-current', 'aria-haspopup', 'aria-live', 'aria-hidden',
];

const noLiteralStringRule = ['error', {
  framework: 'react',
  mode: 'jsx-only',
  'jsx-attributes': { exclude: structuralJsxAttributeExclusions },
}];
...
export default [
  ...
  js.configs.recommended,
  {
    ...
    rules: {
      ...
      'i18next/no-literal-string': noLiteralStringRule,
    },
  },
  {
    files: ['src/**/*.{js,jsx}'],
