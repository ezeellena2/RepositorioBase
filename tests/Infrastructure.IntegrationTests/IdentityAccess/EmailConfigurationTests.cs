...
public sealed class EmailConfigurationTests
{
    ...
    public async Task Actual_generic_worker_host_starts_and_resolves_background_dependencies()
    {
        ...
        npgsqlActivitySource.HasListeners().ShouldBeFalse();
        ...
        var warnings = scope.ServiceProvider.GetRequiredService<DbContextOptions<ApplicationDbContext>>()
            .FindExtension<CoreOptionsExtension>()!
            .WarningsConfiguration;
        warnings.GetBehavior(RelationalEventId.CommandError).ShouldBe(WarningBehavior.Ignore);
        warnings.GetBehavior(CoreEventId.SaveChangesFailed).ShouldBe(WarningBehavior.Ignore);
        warnings.GetBehavior(CoreEventId.QueryIterationFailed).ShouldBe(WarningBehavior.Ignore);
        warnings.GetBehavior(RelationalEventId.ConnectionError).ShouldBe(WarningBehavior.Ignore);
        warnings.GetBehavior(RelationalEventId.TransactionError).ShouldBe(WarningBehavior.Ignore);
        warnings.GetBehavior(RelationalEventId.ExecuteDeleteFailed).ShouldBe(WarningBehavior.Ignore);
        warnings.GetBehavior(RelationalEventId.ExecuteUpdateFailed).ShouldBe(WarningBehavior.Ignore);
        warnings.GetBehavior(RelationalEventId.NonQueryOperationFailed).ShouldBe(WarningBehavior.Ignore);
        warnings.GetBehavior(CoreEventId.OptimisticConcurrencyException).ShouldBe(WarningBehavior.Ignore);
        warnings.GetBehavior(CoreEventId.ExecutionStrategyRetrying).ShouldBe(WarningBehavior.Ignore);
        warnings.GetBehavior(CoreEventId.TypeLoadingErrorWarning).ShouldBe(WarningBehavior.Ignore);
        warnings.GetBehavior(RelationalEventId.BatchExecutorFailedToRollbackToSavepoint).ShouldBe(WarningBehavior.Ignore);
        warnings.GetBehavior(RelationalEventId.BatchExecutorFailedToReleaseSavepoint).ShouldBe(WarningBehavior.Ignore);
