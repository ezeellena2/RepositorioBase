...
public sealed class LocalizationContractTests
{
    ...
    {
        var registeredLanguages = LocalizationRegistry.SupportedLanguages
            .Concat(LocalizationRegistry.InProgressLanguages)
        ...
        foreach (var source in resourceFiles.Where(resource => !IsCultureSpecific(resource.Name, registeredLanguages)))
        {
    ...
    [Test]
    public void Localized_resource_names_are_recognized_from_registered_tags_not_suffix_length()
    {
        IReadOnlySet<string> registeredLanguages = new HashSet<string>(
            ["pt-BR", "zh-Hans"],
            StringComparer.OrdinalIgnoreCase);

        IsCultureSpecific("Emails.pt-BR", registeredLanguages).ShouldBeTrue();
        IsCultureSpecific("Emails.PT-br", registeredLanguages).ShouldBeTrue();
        IsCultureSpecific("Emails.zh-Hans", registeredLanguages).ShouldBeTrue();
        IsCultureSpecific("Emails.fr", registeredLanguages).ShouldBeFalse();
        IsCultureSpecific("Emails.Template", registeredLanguages).ShouldBeFalse();
    }
    ...
    private static bool IsCultureSpecific(string resourceName, IReadOnlySet<string> registeredLanguages)
    {
        var lastSegment = resourceName.LastIndexOf('.');
        return lastSegment >= 0 && registeredLanguages.Contains(resourceName[(lastSegment + 1)..]);
    }
