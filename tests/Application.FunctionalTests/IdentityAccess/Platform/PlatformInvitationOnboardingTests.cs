...
public sealed class PlatformInvitationOnboardingTests : TestBase
{
    ...
    public async Task A_password_the_policy_refuses_is_refused_identically_for_a_real_and_an_unknown_token()
    {
        ...
        var realPolicy = real.Error.ValidationErrors["password"].ShouldHaveSingleItem();
        realPolicy.Code.ShouldBe(ValidationErrorCodes.PasswordPolicy);
        realPolicy.Params.ShouldBeEmpty();
        System.Text.Json.JsonSerializer.Serialize(real.Error.ValidationErrors).ShouldNotContain(PolicyViolatingPassword);
        System.Text.Json.JsonSerializer.Serialize(real.Error.ValidationErrors).ShouldNotContain("Passwords must");
        ...
        var unknownPolicy = unknown.Error.ValidationErrors["password"].ShouldHaveSingleItem();
        unknownPolicy.Code.ShouldBe(realPolicy.Code);
        unknownPolicy.Params.ShouldBeEmpty();
